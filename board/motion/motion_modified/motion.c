...
static void save_three_preview(struct context *cnt, struct image_data *img1, struct image_data *img2, struct image_data *img3)
{
    void * image;
    /* Save preview image pointer */
    image = cnt->imgs.preview_image.image;
    /* Copy all info */
    memcpy(&cnt->imgs.preview_image.image, img1, sizeof(struct image_data));
    /* restore image pointer */
    cnt->imgs.preview_image.image = image;

    /* Copy image */
    memcpy(cnt->imgs.preview_image.image, img1->image, cnt->imgs.size);
	
    void * image2;
    /* Save preview image pointer */
    image2 = cnt->imgs.preview_image2.image;
    /* Copy all info */
    memcpy(&cnt->imgs.preview_image2.image, img2, sizeof(struct image_data));
    /* restore image pointer */
    cnt->imgs.preview_image2.image = image2;

    /* Copy image */
    memcpy(cnt->imgs.preview_image2.image, img2->image, cnt->imgs.size);
	
    void * image3;
    /* Save preview image pointer */
    image3 = cnt->imgs.preview_image3.image;
    /* Copy all info */
    memcpy(&cnt->imgs.preview_image3.image, img3, sizeof(struct image_data));
    /* restore image pointer */
    cnt->imgs.preview_image3.image = image3;

    /* Copy image */
    memcpy(cnt->imgs.preview_image3.image, img3->image, cnt->imgs.size);
}
    ...
    if (img->flags & IMAGE_TRIGGER) {
        ...
        if (cnt->event_nr != cnt->prev_event) {
            ...
            if (cnt->new_img & (NEWIMG_FIRST | NEWIMG_BEST | NEWIMG_CENTER)) 
                save_three_preview(cnt, img, img, img);
...
static void process_image_ring(struct context *cnt, unsigned int max_images)
{
    ...
    do {
        ...
        if (cnt->imgs.image_ring[cnt->imgs.image_ring_out].flags & IMAGE_MOTION) {
			...
			//Modification: Save 3 images to the preview buffer
			int idx1, idx2, idx3;
			idx1 = cnt->imgs.image_ring_out;
			idx2 = cnt->imgs.image_ring_out;
			idx3 = cnt->imgs.image_ring_out;
					
			if(cnt->imgs.image_ring_size >= 3)
			{		
				//Set idx2 to the image before idx1
				if(idx1 > 0)
				{
					idx2 = idx1 - 1;
				}
				...
				{
					idx2 = cnt->imgs.image_ring_size - 1;
				}
					
				//Set idx3 to the image after idx1
				if(idx1 < cnt->imgs.image_ring_size - 1)
				{
					idx3 = idx1 + 1;
				}
				else
			...
			else if(cnt->imgs.image_ring_size == 2)
			{
				idx2 = idx1;
			...
			save_three_preview(cnt, &cnt->imgs.image_ring[idx1], &cnt->imgs.image_ring[idx2], &cnt->imgs.image_ring[idx3]);
                }
            }
        }
...
static int motion_init(struct context *cnt)
{
    ...
    cnt->imgs.preview_image2.image = mymalloc(cnt->imgs.size);
    cnt->imgs.preview_image3.image = mymalloc(cnt->imgs.size);
...
static void motion_cleanup(struct context *cnt)
{
    ...
    if (cnt->imgs.preview_image2.image) {
        free(cnt->imgs.preview_image2.image);
        cnt->imgs.preview_image2.image = NULL;
    }

    if (cnt->imgs.preview_image3.image) {
        free(cnt->imgs.preview_image3.image);
        cnt->imgs.preview_image3.image = NULL;
    }
...
static void *motion_loop(void *arg)
{
    ...
    /***** MOTION LOOP - PREPARE FOR NEW FRAME SECTION *****/
         ...
         */
        frame_buffer_size = cnt->conf.pre_capture + cnt->conf.minimum_motion_frames + 2;
