...
void put_picture(struct context *cnt, char *file, unsigned char *image, int ftype, int notify)
{
    FILE *picture;
...
void preview_save(struct context *cnt)
{
	...
	char previewname2[PATH_MAX];
	char previewname3[PATH_MAX];
    ...
    if (cnt->imgs.preview_image.diffs) {
        ...
        if ((cnt->conf.useextpipe && cnt->extpipe) && !use_imagepath) {
            ...
            put_picture(cnt, previewname, cnt->imgs.preview_image.image , FTYPE_IMAGE, 1);
            ...
            snprintf(previewname, PATH_MAX, "%s/%s%s.%s", cnt->conf.filepath, filename, "-1", imageext(cnt));

            put_picture(cnt, previewname, cnt->imgs.preview_image.image, FTYPE_IMAGE, 0);
			
//			if (cnt->new_img & NEWIMG_CENTER)
//			{
				snprintf(previewname2, PATH_MAX, "%s/%s%s.%s", cnt->conf.filepath, filename, "-2", imageext(cnt));
				snprintf(previewname3, PATH_MAX, "%s/%s%s.%s", cnt->conf.filepath, filename, "-3", imageext(cnt));
			 
				put_picture(cnt, previewname2, cnt->imgs.preview_image2.image, FTYPE_IMAGE, 0);
				put_picture(cnt, previewname3, cnt->imgs.preview_image3.image, FTYPE_IMAGE, 1);
//			}
        }
