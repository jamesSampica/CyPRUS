	...
	public static byte[] packetToBytes(Packet object){
		...
		try {
		  ...
		  bos.close();
			...
			//e.printStackTrace();
			Logger.getLogger(SerializationUtils.class.getName()).log(Level.WARNING,  "packetToBytes IOException: " + e.getMessage());
		}
	...
	public static Packet bytesToPacket(byte[] objectBytes){
		...
		try {
		  ...
		  in.close();
			...
			// TODO Auto-generated catch block
			//e.printStackTrace();
			Logger.getLogger(SerializationUtils.class.getName()).log(Level.SEVERE,  "bytesToPacket ClassNotFoundException: " + e.getMessage());
		}
