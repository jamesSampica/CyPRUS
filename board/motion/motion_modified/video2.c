...
/*static int v4l2_set_control(src_v4l2_t * vid_source, u32 cid, int value)
{
    ...
    {
         MOTION_LOG(WRN, TYPE_VIDEO, NO_ERRNO, "%s: Vid source does not have controls. Not setting anything.");
...
static void v4l2_picture_controls(struct context *cnt, struct video_dev *viddev)
{
    ...
    //MOTION_LOG(WRN, TYPE_VIDEO, NO_ERRNO, "%s: Setting picture controls");
    
    //modification
    viddev->exposure = 256;
    //v4l2_set_control(vid_source, V4L2_CID_EXPOSURE, viddev->exposure);
    ...
    control.value = viddev->exposure;
    
    int fd = open("/dev/media0", O_RDWR | O_NONBLOCK);

    //ret = ioctl(vid_source->fd, VIDIOC_S_CTRL, &control);
    ret = ioctl(fd, VIDIOC_S_CTRL, &control);
    if(ret == -1)
    {
        MOTION_LOG(ERR, TYPE_VIDEO, SHOW_ERRNO, "%s: Error setting exposure control- %s", strerror(errno));
    }
                 
    close(fd);
    viddev->gamma = 16;
    //v4l2_set_control(vid_source, V4L2_CID_GAMMA, viddev->gamma);
    //end
*/
              ...
              int input, int norm, unsigned long freq, int tuner_number)
{
    ...
    vid_source->pframe = -1;
    //vid_source->controls = 1; //modification
